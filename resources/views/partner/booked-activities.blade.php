...
$( "#slider-range" ).on( "slidechange", function( event, ui ) { $("#activity-calender-result").html(""); getActivities(); } );
...
function getActivities()
{
    ...
    request_ar['from_time'] = from_time+":00";
    request_ar['to_time'] = to_time+":00";
	...
	request_ar['dealer_id'] = dealer;
	request_ar['latitude'] = "";
	request_ar['longitude'] = "";
...
function getActivitiesREST( json_data )
		{
			$(".loader-back").show();
			$.ajax({
                url: "{{route('partner_booked_activity_url')}}",
                type: "post",
