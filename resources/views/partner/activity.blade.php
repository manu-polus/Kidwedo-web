...
<section class="container-fluid register-bg">
	<div class="container">
		<div class="activity-view  add-activity-outer mb-20 w-70 m-auto">
			...
			<form method="POST" action="{{ route('partneractivity') }}" enctype="multipart/form-data" id="add-event-form">
								@csrf
			<div class="row p-15">
				<div class="col-lg-6">
				...
				<div class="col-lg-6">
					<div class="form-group">
						...
						<textarea class="custom-textarea"  placeholder="Beschreibung" name="description">{{ old('description') }}</textarea>
					...
					<div class="form-group">
						...
						<select class="custom-select custom-select-box" name="category">
							...
							@foreach($categories as $category)
											<option {{ (old('category') == $category->id) ? 'selected' : '' }} value = "{{ $category->id }}">{{ $category->description }}</option>
							@endforeach
						</select>
...
$("#add-activity-submit").on("click", function(event){
