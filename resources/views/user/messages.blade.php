				...
				<div class="col-lg-12  my-message-outer">
					<div class="flex w-100">
						...
						<div>
							...
							<p>{{ $message['time'] }} <b class="msg-from">{{ $message['name'] }}</b> |@if($message['from_user_id'] != Auth::user()->id)<span class="msg-reply"  data-toggle="modal" data-target="#messageModal" data-profile='<?php echo $formdata; ?>'>Antworten</span>|@endif<a href="{{ route('message.delete',['id' => $message['id']]) }}" onclick="return delete_confirm()"><span class="msg-delete">Löschen</span></a></p>
						</div>
					</div>
						...
						@endphp
							<div class="col-lg-12"><!--loop-child-->
								...
								<div>
									...
									<p>{{ $child['time'] }} <b class="msg-from">{{ $child['name'] }}</b> |@if($child['from_user_id'] != Auth::user()->id)<span class="msg-reply"  data-toggle="modal" data-target="#messageModal" data-profile='<?php echo $formdata; ?>'>Antworten</span>|@endif<a href="{{ route('message.delete',['id' => $child['id']]) }}" onclick="return delete_confirm()"><span class="msg-delete">Löschen</span></a></p>
								</div>
							</div>
						</div>
						@endforeach
